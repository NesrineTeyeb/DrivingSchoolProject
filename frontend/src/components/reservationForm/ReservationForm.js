...
const ReservationForm = () => {
  ...
  const checkExistingReservation = async () => {
    try {
      ...
      setHasReservation(false); // En cas d'erreur, assurez-vous que l'état est mis à false
    }
  };
  ...
  <Card className="text-center">
    ...
    <Card.Body>
      ...
      <Card.Text>
        <strong>Date:</strong> {new Date(existingReservation?.date).toLocaleDateString()} <br />
        <strong>Heure:</strong> {existingReservation?.time} <br />
        <strong>Type de leçon:</strong> {existingReservation?.lessonType}
      </Card.Text>
