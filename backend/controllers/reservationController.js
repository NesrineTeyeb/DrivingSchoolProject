...
exports.createReservation = async (req, res) => {
  try {
    ...
    // Vérifier si l'utilisateur a déjà une réservation future
    const existingReservation = await Reservation.findOne({
      email,
      date: { $gte: new Date() }, // Vérifie si une réservation future existe
    });

    if (existingReservation) {
      return res.status(400).json({ error: "Vous avez déjà une réservation en cours." });
    }

    // Création de la réservation si aucune réservation existante
    const newReservation = new Reservation(req.body);
    await newReservation.save();
